if(@ARGV != 3) {
	print "Usage: perl convertCORD19SemRepToTTL.pl cord19_uid_docid.txt CORD-19.SemRep cord19_semrep\n";
	exit 1;
}

my $semrep = "<https://semrep.nlm.nih.gov/";
my $base = "<https://semrep.nlm.nih.gov/covid19/cord19#";
my $int = "^^<http://www.w3.org/2001/XMLSchema#int>";
my %cordids = ();
my $cordid_file = $ARGV[0];
my $input_dir = $ARGV[1];
$input_dir =~ s/\/$//;
my $output_dir = $ARGV[2];
$output_dir =~ s/\/$//;
open(CORD, $cordid_file) or die "Can't open $ARGV[0]\n";
while($line=<CORD>) {
	chomp($line);
	($cord_uid, $doc_id) = (split(/\t/, $line))[0, 1];
	$cordids{$cord_uid} = $doc_id;
}
close(CORD);

for my $cord_uid (keys(%cordids)) {
	$ttl = convertSemRepToTTL($input_dir, $cord_uid, $cordids{$cord_uid}, $output_dir);
	if($ttl) {
		#print $ttl;
		$ttl_file = $output_dir."/".$cord_uid.".ttl";
...
sub convertSemRepToTTL {
	my ($input_dir, $cord_uid, $doc_id, $output_dir) = @_;
	#print $cord_uid."\t".$doc_id."\n";
	$semrep_file = $input_dir."/".$cord_uid.".txt.out";
	#print $semrep_file."\n";
	if(-e $semrep_file) {
		$ttl = "[\n";
		$ttl .= "	".$base."_id> \"".$cord_uid."\" ;\n"; 
		$ttl .= "	".$base."id> \"".$doc_id."\" ;\n"; 
